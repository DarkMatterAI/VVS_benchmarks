...
SAVE_DIR = Path("/code/internal/bbknn/poses")
...
# ────────────────────────────────────────────────────────────────────────────
# save helper
# def _save_docked_mol(docked, smiles, save_dir: Path):
#     """Write *docked* pose to ``save_dir/<md5>.sdf``, with SMILES in SD data."""
#     save_dir = Path(save_dir)
#     save_dir.mkdir(parents=True, exist_ok=True)
#     fname = hashlib.md5(smiles.encode()).hexdigest() + ".sdf"
#     oechem.OESetSDData(docked, "SMILES", smiles)
#     ofs = oechem.oemolostream(str(save_dir / fname))
#     oechem.OEWriteMolecule(ofs, docked)
#     ofs.close()

def _save_docked_mol(docked, smiles, save_dir: Path, save_name: str = None):
    """Write *docked* pose to ``save_dir/<name>.sdf``, with SMILES in SD data.

    If *save_name* is given it is used as the stem; otherwise falls back
    to the MD5 hex-digest of *smiles*.
    """
    save_dir = Path(save_dir)
    save_dir.mkdir(parents=True, exist_ok=True)
    if save_name is not None:
        fname = save_name if save_name.endswith(".sdf") else save_name + ".sdf"
    else:
        fname = hashlib.md5(smiles.encode()).hexdigest() + ".sdf"
    oechem.OESetSDData(docked, "SMILES", smiles)
    ofs = oechem.oemolostream(str(save_dir / fname))
...
def _dock_one(smiles: str, dock: oedocking.OEDock, max_conf=10,
    ...
    score = -float(oechem.OEGetSDData(docked, tag))      # higher = better

    if save_dir is not None:
        _save_docked_mol(docked, smiles, save_dir, save_name=save_name)
...
def _batch(
    ...
    protein_name: str = None,                 # e.g. "2zdt" – enables pose saving
):
    out = []
    for rec in request:
        try:
            ...
            # Build extra kwargs (save_dir, save_name) only for docking
            kwargs = {}
            if protein_name is not None:
                save_subdir = rec.get("save_dir", None)
                if save_subdir is not None:
                    kwargs["save_dir"] = SAVE_DIR / protein_name / save_subdir
                    save_name = rec.get("save_name", None)
                    if save_name is not None:
                        kwargs["save_name"] = save_name

            s = func(smi, resource, **kwargs)
...
def dock_2zdt(request):
    ...
    return _guard(
        ...
        lambda: _batch(request, _dock_one, _read_grid(FILES[name]),
                       protein_name="2zdt"), # 2zdt protein name
    )
...
# # paths
# BASE_DIR = Path("/code/processed/openeye")
# SAVE_DIR = Path("/code/openeye_structures")
